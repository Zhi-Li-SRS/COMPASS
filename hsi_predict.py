...
class HSIPredictor:
    def __init__(self, model_path: str, library_path: str, target: List[str] = None):
        """
        Args:
        ...
        # Set target classes for prediction (if None, use all non-background classes)
        if target is None:
            self.target = self.non_bg_class_names
            print(f"No target specified, using all non-background classes")
        else:
            for name in target:
                if name not in self.class_names:
                    print(f"Warning: Target '{name}' not found in model classes")
            
            # Keep only valid target classes
            self.target = [name for name in target if name in self.class_names and self.class_names.index(name) != self.bg_idx]
            print(f"Using {len(self.target)} specified target classes: {self.target}")
    ...
    def predict_hsi(self, image_path: str, output_dir: str):
        ...
        if self.bg_idx is not None:
            ...
            bg_prob_threshold = self.prob_thresholds.get("background", 0.95)
...
def main():
    ...
    parser.add_argument(
        "--library_path", 
        type=str, 
        default="Raman_dataset/ten_molecules_dataset/train_reference.csv", 
